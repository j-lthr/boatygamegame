...
pub fn cast_shotgun(
    mut cast_events: EventReader<CastEvent<Shotgun>>,
    ...
    for cast_event in cast_events.read() {
        if let Ok(caster_transform) = caster_query.get(cast_event.caster) {
            ...
            for _ in 0..cast_event.ability.bullet_count {
                // Calculate bullet direction with spread
                let direction = (cast_event.params.target_position - caster_transform.translation)
                    .normalize()
                    .with_y(0.0)
                    + Vec3::new(fastrand::f32() - 0.5, 0.0, fastrand::f32() - 0.5) * cast_event.ability.spread;
...
impl Ability for Shotgun {
    type CastParams = ShotgunParams;

    fn add_systems(app: &mut bevy::app::App) {
        app.add_systems(Update, cast_shotgun);
    }
}