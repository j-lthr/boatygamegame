...
use crate::{init::{DespawnOnReset, GameInit}, player::Player};
...
pub fn update_cursor(
    ...
    player: Query<&Transform, (With<Player>, Without<Cursor>)>,
    ...
    if let Some(position) = ray.intersect_plane(Vec3::ZERO, InfinitePlane3d { normal: Dir3::Y })
        .map(|intersection| ray.origin + ray.direction * intersection) {
            ...
            cursor_transform.rotation = cursor_transform.rotation.slerp(target_transform.rotation, 10.0 * time.delta_secs());
    }   
...
pub fn spawn_cursor(mut commands: Commands,
    ...
    mut materials: ResMut<Assets<StandardMaterial>>, mut window: Query<&mut Window>) -> Result{
        commands.spawn((
            Cursor,
            DespawnOnReset,
            Transform::from_scale(Vec3::splat(0.7)),
            Mesh3d(meshes.add(Triangle3d::new(Vec3::new(0.5,0.0,-0.5), Vec3::new(-0.5,0.0,-0.5), Vec3::new(0.0,0.0,0.5)))),
