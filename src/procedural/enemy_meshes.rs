...
pub fn generate_crystal_mesh(radius: f32, spikes: usize) -> Mesh {
    ...
    for i in 0..spikes {
        ...
        positions.push(Vec3::new(side_angle1.cos() * radius * 0.7, radius * 0.2, side_angle1.sin() * radius * 0.7));
        positions.push(Vec3::new(side_angle2.cos() * radius * 0.7, radius * 0.2, side_angle2.sin() * radius * 0.7));
    }
    ...
    for i in 0..spikes {
        ...
        indices.extend_from_slice(&[base_idx as u32, (base_idx + 1) as u32, (base_idx + 2) as u32]);
        indices.extend_from_slice(&[base_idx as u32, (base_idx + 2) as u32, (base_idx + 3) as u32]);
        indices.extend_from_slice(&[(base_idx + 1) as u32, (base_idx + 2) as u32, (base_idx + 3) as u32]);
    }
    ...
    for segment in 0..segments {
        for side in 0..sides {
            ...
            indices.extend_from_slice(&[
                current_base as u32, next_base as u32, current_next as u32,
            ]);
            indices.extend_from_slice(&[
                current_next as u32, next_base as u32, next_next as u32,
            ]);
        }
    }
...
pub fn generate_angular_mesh(size: f32) -> Mesh {
    ...
    let faces = vec![
        ...
        [5, 6, 2], [6, 7, 3], [7, 8, 4], [8, 5, 1],
        
        // Bottom pyramid
        [9, 11, 10], [9, 12, 11], [9, 13, 12], [9, 10, 13],
        
        // Bottom connecting faces  
        [10, 11, 5], [11, 12, 6], [12, 13, 7], [13, 10, 8],
        [11, 6, 5], [12, 7, 6], [13, 8, 7], [10, 5, 8],
    ];
...
pub fn generate_twisted_pillar(radius: f32, height: f32, twist_amount: f32) -> Mesh {
    ...
    for segment in 0..segments {
        for side in 0..sides {
            ...
            indices.extend_from_slice(&[
                current as u32, next_segment as u32, current_next as u32,
            ]);
            indices.extend_from_slice(&[
                current_next as u32, next_segment as u32, next_segment_next as u32,
            ]);
        }
    }
