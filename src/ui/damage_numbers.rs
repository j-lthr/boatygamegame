...
impl Plugin for DamageNumbersPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(Update, (spawn_damage_numbers, update_damage_numbers, cleanup_damage_numbers));
    }
}
...
pub fn update_damage_numbers(
    ...
    for (mut transform, mut damage_number, mut text_color) in &mut damage_number_query {
        ...
        let progress = damage_number.lifetime.elapsed_secs() / damage_number.lifetime.duration().as_secs_f32();
        ...
        let world_pos = damage_number.initial_position + Vec3::new(0.0, damage_number.rise_speed * progress, 0.0);
